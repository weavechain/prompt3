...
const decodeString = (encoded, key, iv) => {
	...
	const result = new TextDecoder()
		.decode(new Uint8Array(wordToByteArray(decrypted)))
		.trim();
...
const encodeStringFor = (plaintext, fromPrivateKey, toPubKey, iv) => {
	...
	const bytes =
		typeof fromPrivateKey === "string"
			? fromPrivateKey.startsWith("0x")
				? fromHex(fromPrivateKey.substr(2))
				: base58_to_binary(fromPrivateKey)
			: fromPrivateKey;
	const clientKeys = ecc.keyFromPrivate(
		bytes.length === 33 ? bytes.subarray(1) : bytes
	);
	const secretKey = fromHexU(
		clientKeys.derive(pub.getPublic()).toString(16),
		32
	);
...
const decodeStringFrom = (encoded, fromPubKey, toPrivateKey, iv) => {
	const wkey = enc.Hex.parse(
		typeof fromPubKey === "string" ? fromPubKey : toHex(fromPubKey)
	);
	const wiv = enc.Hex.parse(typeof iv === "string" ? iv : toHex(iv));
	...
	const pub = ecc.keyFromPublic(pubKey);
	const bytes =
		typeof toPrivateKey === "string"
			? toPrivateKey.startsWith("0x")
				? fromHex(fromPubKey.substr(2))
				: base58_to_binary(toPrivateKey)
			: toPrivateKey;
	const clientKeys = ecc.keyFromPrivate(
		bytes.length === 33 ? bytes.subarray(1) : bytes
	);
